...
export default function GlobalAddTributeForm() {
  ...
  useEffect(() => {
    const checkHash = () => {
      ...
      if (hash === '#add-tribute') {
        // Scroll page to top for consistent form visibility
        // This ensures form is fully visible regardless of scroll position
  ...
  return createPortal(
    <>
      ...
      {/* Form Container - Centered on both mobile and desktop using flexbox */}
      <div
        ref={formRef}
        className="fixed inset-0 z-[100000] flex items-center justify-center p-4 md:p-6 animate-fadeIn pointer-events-none"
        role="dialog"
        aria-modal="true"
        aria-labelledby="tribute-form-title"
        tabIndex={-1}
      >
        {/* Inner scroll container with actual form dimensions */}
        <div className="w-full max-w-[600px] max-h-[90vh] md:max-h-[calc(100vh-80px)] overflow-y-auto animate-scaleIn pointer-events-auto">
